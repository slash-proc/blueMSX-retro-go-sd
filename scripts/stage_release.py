#!/usr/bin/env python3
"""Stage Retro-Go SD release assets for blueMSX.

Builds:
  1. SD install zip:   <stem>-<tag>.zip       → cores/<packed.bin> + bios/msx/*
  2. Debug symbols zip: <stem>-<tag>-debug.zip → ELF, map, README

No loose .bin assets on the GitHub Release (unzip the install archive onto
the SD root). Extracts release notes from CHANGELOG.md for the tag.

Usage:
  python3 scripts/stage_release.py --bin blueMSX.bin --tag v0.0.1 --out release \\
      --elf build/core/msx_core.elf --map build/core/msx_core.map
"""
...
def build_release_notes(
    *,
    ...
    if project_kind == "core":
        ...
        lines.append(
            "- Unzip keeps `cores/` + `bios/msx/` layout — copy both onto the SD card."
        )
    else:
...
def stage_release(
    *,
    ...
    write_zip(
        archive_path,
        [(sd_bin, f"{sd_dir}/{packed_name}"), *bios_pairs],
    )

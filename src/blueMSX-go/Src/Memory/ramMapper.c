...
#ifdef TARGET_GNW
/* One WRAM allocation for all models (survives dtc_init). Max MSX2+ size. */
UInt8 *msxSharedMainRam;
int    msxSharedMainRamSize;

static UInt8 *msxGetSharedMainRam(int size)
{
    if (msxSharedMainRam == NULL) {
        int need = size < (256 * 1024) ? (256 * 1024) : size;
        msxSharedMainRam = (UInt8 *)ram_malloc(need);
        msxSharedMainRamSize = msxSharedMainRam ? need : 0;
    }
    if (msxSharedMainRam == NULL || size > msxSharedMainRamSize) {
        return NULL;
    }
    return msxSharedMainRam;
}
#endif
...
#else
    rm = dtc_malloc(sizeof(RamMapper));
    rm->ramData = msxGetSharedMainRam(size);
    if (rm->ramData == NULL) {
        return 0;
    }
#endif

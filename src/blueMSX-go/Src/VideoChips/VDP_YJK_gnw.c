/*****************************************************************************
** GNW: YJK -> RGB565 via subsampled DTCM lookup table (table-only hot path).
**
** msxYjkColorLo[32][32][32] in DTCM via dtc_malloc (~64 KiB), filled once by
** msxYjkColorInit(). (AHB is too small for this table on SD Retro-Go; ITCM
** is reserved for hot code.) Runtime: one indexed load — J and K quantized
** by >>1 (nearest even sample).
*****************************************************************************/
#include "VDP_MSX.h"
...
void msxYjkColorInit(void)
{
    ...
    msxYjkColorLo = dtc_malloc(MSX_YJK_LO_SIZE);

...
/* Last: its macros rewrite ACTIVE_FILE / ram_start / common_emu_state. */
...
static bool update_frequency_cb(odroid_dialog_choice_t *option, odroid_dialog_event_t event, uint32_t repeat)
{
    ...
    switch (selected_frequency_index) {
        case FREQUENCY_VDP_AUTO:
            strcpy(option->value, gw_i18n(i18n_freq_auto));
            break;
        case FREQUENCY_VDP_50HZ: // Force 50Hz
            strcpy(option->value, gw_i18n(i18n_freq_50));
            break;
        case FREQUENCY_VDP_60HZ: // Force 60Hz
            strcpy(option->value, gw_i18n(i18n_freq_60));
            break;
    }
...
static bool update_msx_cb(odroid_dialog_choice_t *option, odroid_dialog_event_t event, uint32_t repeat)
{
    ...
    switch (selected_msx_index) {
        case 0: // MSX1;
            msx2_dif = 0;
            strcpy(option->value, gw_i18n(i18n_msx1_eur));
            break;
        case 1: // MSX2;
            msx2_dif = 10;
            strcpy(option->value, gw_i18n(i18n_msx2_eur));
            break;
        case 2: // MSX2+;
            msx2_dif = 10;
            strcpy(option->value, gw_i18n(i18n_msx2_jp));
            break;
    }
    ...
    if (event == ODROID_DIALOG_ENTER) {
        boardInfo.destroy();
        boardDestroy();
        /* Was ahb_init() in the firmware build. A dynamic core must not
         * rewind the AHB heap: that pool is the firmware's, shared with the
         * launcher, and the ABI deliberately exposes no reset for it. The
         * pool this core actually allocated the old machine out of is
         * RAM_EMU, so that is what gets forgotten before the new machine is
         * built. */
        ram_init();
        itc_init();
...
static void createOptionMenu(odroid_dialog_choice_t *options) {
    ...
    if (msx_game_type == MSX_GAME_DISK) {
        ...
        options[index].label = gw_i18n(i18n_change_dsk);

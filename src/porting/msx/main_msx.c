...
/* Set on option confirm; rebuild runs in the main loop (not inside the
 * overlay callback) so FatFs/SPI is not re-entered mid-menu. */
static int pending_msx_type = -1;
...
static bool update_msx_cb(odroid_dialog_choice_t *option, odroid_dialog_event_t event, uint32_t repeat)
{
    ...
    if (event == ODROID_DIALOG_ENTER) {
        pending_msx_type = selected_msx_index;
    }
    return event == ODROID_DIALOG_ENTER;
}
...
/* File-scope (not nested): clang host build rejects GCC nested functions. */
static void msx_blit_frame(void)
{
...
static void msx_blit_with_overlay(void)
{
    msx_blit_frame();
...
#endif
    while (1) {
        if (pending_msx_type >= 0) {
            int type = pending_msx_type;
            pending_msx_type = -1;
#if SD_CARD == 1
            if (msx_game_type == MSX_GAME_DISK)
                diskCloseDrive(0);
        ...
        common_emu_input_loop(&joystick, options, &msx_blit_with_overlay);
